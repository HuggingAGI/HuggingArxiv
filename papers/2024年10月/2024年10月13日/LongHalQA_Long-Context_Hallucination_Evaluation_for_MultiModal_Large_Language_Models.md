# LongHalQA：针对多模态大型语言模型的长上下文幻觉进行评估
...
> 幻觉现象，即多模态大型语言模型 (MLLM) 生成看似合理但与图像不符的文本，已成为 MLLM 应用的一大难题。现有基准通过提出区分性问题或引入 LLM 评估器来衡量幻觉水平，但这些方法存在局限性。我们提出了 LongHalQA，一个无需 LLM 的幻觉基准，包含 6K 个长且复杂的幻觉文本，由 GPT4V 生成，与现实场景高度一致。LongHalQA 引入了幻觉辨别和幻觉补全两个新任务，统一了区分性和生成性评估，实现更可靠高效的评估。此外，我们提出了一种先进管道，便于未来幻觉基准的建设。实验揭示了 MLLM 在处理长复杂文本幻觉时的新挑战。数据集和评估代码已公开。
