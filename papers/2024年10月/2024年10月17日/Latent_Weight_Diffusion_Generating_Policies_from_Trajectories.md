...
> 随着开源机器人数据的增多，模仿学习在机器人操作和移动中崭露头角。当前，大型通用策略通过扩散模型预测控制或轨迹，具备学习多模态动作分布的优势。然而，通用性带来模型规模增大和推理速度减慢的代价。此外，扩散策略在性能和动作范围间存在权衡，较少扩散查询导致更大轨迹误差。因此，高频推理成为常见做法，受限于机器人计算能力。为应对这些挑战，我们提出潜在权重扩散（LWD），通过扩散学习机器人任务策略分布而非轨迹。LWD将演示轨迹编码至潜在空间，再由超网络解码为策略，并在潜在空间内应用扩散去噪模型。实验表明，LWD能重建原始策略行为，推理时策略网络显著缩小，扩散查询减少。在Metaworld MT10基准测试中，LWD成功率超越普通多任务策略，模型大小缩小至1/18。此外，LWD生成闭环策略，在长动作范围设置中表现更优，回滚时扩散查询减少。
