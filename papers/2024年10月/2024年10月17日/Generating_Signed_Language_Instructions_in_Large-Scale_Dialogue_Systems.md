...
> 我们推出了一款目标导向的对话AI系统，首次在全球多模态对话平台上实现了美国手语（ASL）指令的增强功能。通过触摸界面，系统接收用户输入，并利用检索技术和认知词汇翻译，流畅生成ASL指令。核心设计包括一个由大型语言模型驱动的符号翻译模块，以及一个基于令牌的视频检索系统，用于从食谱和wikiHow指南中提取教学内容。开发过程中，我们深入社区，融合了聋人和听力障碍者社区及认知与ASL学习专家的见解。用户反馈验证了手语指令的有效性，评分与非手语版本系统相当。此外，系统在检索准确性和文本生成质量上表现卓越，通过BERTScore等指标评估。代码库和数据集已公开在https://github.com/Merterm/signed-dialogue，手语指令视频检索系统演示可在https://huggingface.co/spaces/merterm/signed-instructions查看。
