# ForgeryGPT：一款多模态大型语言模型，专为可解释的图像伪造检测与定位而生。
...
> 多模态大型语言模型如 GPT4o 在视觉推理和解释生成方面表现出色，但在图像伪造检测和定位 (IFDL) 这一关键任务中仍面临挑战。现有方法多局限于低级线索，仅提供单一判断。为此，我们提出 ForgeryGPT，通过捕捉伪造图像的高阶知识相关性，结合定制 LLM 架构，实现可解释生成和交互对话。ForgeryGPT 集成 Mask-Aware Forgery Extractor，从图像中提取精确伪造信息，增强像素级理解。通过三阶段训练策略和专用数据集，提升伪造检测和指令跟随能力。实验证明其有效性。
