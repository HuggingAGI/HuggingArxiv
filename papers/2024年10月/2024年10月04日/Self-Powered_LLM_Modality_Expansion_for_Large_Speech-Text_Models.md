...
> 大型语言模型 (LLM) 在多任务中表现出色，显示出整合语音能力后扩展为大型语音-文本模型 (LSM) 的潜力。尽管语音-文本统一预训练和多模态数据指令微调带来诸多优势，但这些方法往往资源消耗巨大且易过度适应特定任务。本研究通过优化普通指令微调的局限性，探索如何更有效地利用语音数据集进行 LSM 训练。我们发现 LSM 存在语音锚定偏差，即过度依赖语音输入而忽视文本指令。为此，我们提出自供电 LSM，利用模型生成的增强语音识别数据进行更精准的指令微调。实验证明，自供电 LSM 有效缓解了语音锚定偏差，提升了语音与文本模态的融合效果。相关数据、代码及脚本已公开，详见 https://github.com/ytf-philp/Self-powered-LSM。
