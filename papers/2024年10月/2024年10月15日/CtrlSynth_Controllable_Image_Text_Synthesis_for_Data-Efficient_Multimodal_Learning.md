...
> 预训练鲁棒的视觉或多模态基础模型（如CLIP）依赖于可能存在噪声、对齐问题和长尾分布的大规模数据集。尽管先前的工作通过生成合成样本展示了增强数据集的潜力，但这些方法仅限于特定领域的即席用例，且由于缺乏对合成过程的细粒度控制，数据多样性受限。为此，我们设计了可控的图像-文本合成管道CtrlSynth，旨在实现数据高效和鲁棒的多模态学习。CtrlSynth的核心在于将图像的视觉语义分解为基本元素，并根据用户指定的控制策略（如移除、添加或替换）重新组合，从而合成图像或文本。通过分解与重新组合功能，用户可以定义定制策略，以细粒度方式控制数据合成。此外，CtrlSynth利用预训练基础模型（如大型语言模型或扩散模型）的能力，确保合成样本自然且多样化。作为一个闭环、无需训练且模块化的框架，CtrlSynth易于支持不同预训练模型。实验结果表明，CtrlSynth显著提升了CLIP模型在零样本分类、图像-文本检索和组合推理任务中的性能。
