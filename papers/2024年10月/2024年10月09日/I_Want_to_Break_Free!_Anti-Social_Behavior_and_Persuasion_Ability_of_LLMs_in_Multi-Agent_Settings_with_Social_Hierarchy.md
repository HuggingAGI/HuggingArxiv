# 渴望自由！在多代理社会层次结构中，LLM 的反社会行为及其说服能力探究
...
> 随着基于 LLM 的代理日益自主并自由互动，研究其交互变得关键，以预测涌现现象和潜在风险。受斯坦福监狱实验启发，我们研究了在严格社会等级背景下 LLM 代理的交互模式，特别是说服和反社会行为。通过 200 个实验场景和 2,000 次机器对话，我们发现：一些模型在多代理设置中持续失败；成功交互的模型中，目标主要影响说服力，对反社会行为影响甚微；代理角色，尤其是守卫性格，影响说服和反社会行为；即使未明确提示，角色分配也能引发反社会行为。这些发现对 LLM 代理开发和社会影响辩论具有重要意义。
