# 通过多轮偏好优化，提升多模态 LLM 在视频字幕生成中的细节与准确性。
...
> 视频蕴含丰富信息，生成详尽准确的自然语言描述是视频理解的核心。本文介绍的视频-SALMONN 2，是一款结合低秩适应（LoRA）的先进音视频大型语言模型（LLM），通过定向偏好优化（DPO）提升视频（含音频）字幕质量。我们设计了新指标评估描述的完整性与准确性，并利用DPO优化。为进一步提升训练效果，我们创新性地引入了多轮DPO（mrDPO）方法，包括定期更新DPO参考模型、合并重置LoRA模块以模拟参数更新，并借助真实视频字幕稳定训练过程。针对mrDPO可能导致的非字幕能力遗忘问题，我们提出重生调优，即用mrDPO训练模型生成的字幕作为监督标签微调预DPO LLM。实验结果显示，mrDPO大幅提升视频-SALMONN 2的字幕准确性，分别降低全局和局部错误率40%和20%，重复率减少35%。最终的70亿参数视频-SALMONN 2模型，在视频字幕任务中超越GPT-4o和Gemini-1.5-Pro等顶尖模型，同时在同类模型中保持视频问答基准的领先地位。项目接受后，我们将公开代码、模型检查点及训练测试数据。演示可访问\href{https://video-salmonn-2.github.io}{https://video-salmonn-2.github.io}。
