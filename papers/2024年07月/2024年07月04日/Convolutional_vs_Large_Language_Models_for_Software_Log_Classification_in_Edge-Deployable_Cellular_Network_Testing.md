...
> 电信行业中，复杂的网络模拟器如VIAVI TM500生成的软件日志极为繁杂，包含数万行与自然语言相去甚远的文本。唯有专业工程师能解读这些日志，解决测试中的缺陷。AI虽为自动化缺陷分类带来希望，可能大幅节省公司收入，但现有大型语言模型（LLMs）在此领域存在明显短板，如上下文窗口受限、对非自然语言文本适用性差及高推理成本。为此，我们设计了一种紧凑的卷积神经网络（CNN）架构，提供长达200,000字符的上下文窗口，并在日志分类中达到96%以上的准确率。该模型能自动识别并分类测试缺陷，替代了以往需专家手动处理的过程。我们测试了多种LLMs，如LLaMA2-7B、Mixtral 8x7B等，并验证了它们在特定应用中的不足。尽管轻巧，我们的CNN在电信日志分类上表现卓越，成本更低。此AI模型无需专用硬件，即可在边缘设备上部署，广泛适用于各行业软件日志。
