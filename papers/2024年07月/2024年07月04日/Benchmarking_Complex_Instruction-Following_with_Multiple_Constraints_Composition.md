...
> 随着大型语言模型（LLM）能力的不断提升，它们在处理现实世界中的复杂指令方面发挥着越来越重要的作用。然而，评估LLM遵循复杂指令的能力成为一个关键问题。现有的评估基准主要关注单一约束，忽略了复杂指令中约束的组合。为此，我们推出了ComplexBench，这是一个全面评估LLM处理多约束复杂指令能力的基准。我们设计了一个包含4种约束类型、19个约束维度和4种组合类型的分层分类法，并精心收集了相关数据集。通过规则增强的LLM评估器，我们确保了评估的可靠性，并根据约束组合的依赖结构得出最终评分。ComplexBench揭示了现有LLM在处理复杂多约束指令时的明显不足。
