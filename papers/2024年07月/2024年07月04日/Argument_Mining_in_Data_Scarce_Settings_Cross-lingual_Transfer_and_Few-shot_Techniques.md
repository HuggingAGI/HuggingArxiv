# 在数据稀缺环境下，论点挖掘借助跨语言迁移和少量样本技术，探索新的可能性。
...
> 近期研究探索了多种策略以解决全球多数语言缺乏人工标注数据的问题。最有效的方法包括：（i）多语言预训练模型的跨语言迁移（模型迁移），（ii）数据翻译与标签映射（数据迁移），以及（iii）利用预训练模型少样本能力的基于提示学习（少样本）。以往研究认为模型迁移胜过数据迁移，且提示驱动的少样本学习优于模型微调。然而，本文实证表明，在需要识别复杂话语结构的论点挖掘任务中，这些先前结论并不成立。我们发现，数据迁移效果优于模型迁移，且微调技术超越了少样本方法。数据集领域对数据迁移至关重要，而任务类型与采样策略则对少样本学习影响显著。
...
![在数据稀缺环境下，论点挖掘借助跨语言迁移和少量样本技术，探索新的可能性。](../../../paper_images/2407.03748/neoplasm_ft_entlm_pointplot.png)
