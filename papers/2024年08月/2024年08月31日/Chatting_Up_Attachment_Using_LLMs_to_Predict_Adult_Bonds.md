...
> 在医疗领域，数据的获取充满挑战，导致AI技术的应用进展缓慢且风险高。我们探索了通过大型语言模型（如GPT-4和Claude 3 Opus）生成的合成数据来克服这一难题的可能性。我们设计了模拟不同背景、童年记忆和依恋风格的成年人代理，并让他们参与模拟的成人依恋访谈（AAI）。通过这些代理的回答，我们训练模型来预测他们的依恋风格。我们使用心理健康专业人员分析和标记的真实访谈转录数据来评估模型。结果显示，仅用合成数据训练的模型性能与用真实数据训练的模型相当。此外，尽管合成数据与真实数据的原始嵌入存在差异，但通过引入未标记的真实数据和标准化处理，我们成功缩小了这一差距，提升了预测准确性，这一改进得到了定性分析的验证。
