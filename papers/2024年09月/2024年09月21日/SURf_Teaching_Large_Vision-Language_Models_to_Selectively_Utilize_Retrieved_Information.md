...
> 大型视觉-语言模型 (LVLMs) 在计算机视觉与自然语言处理的交汇处发挥着关键作用。然而，LVLMs 的检索增强生成 (RAG) 能力尚未完全释放。现有研究要么局限于文本模态，要么仅针对特定任务。此外，多数 LVLMs 在处理检索信息时表现不佳，容易受到无关或误导性参考的影响。为此，我们设计了一个自精炼框架，名为 SURf，旨在提升 LVLMs 选择性利用检索信息的能力。具体而言，当 LVLM 主干回答错误时，我们收集有助于纠正答案的正参考和无助于纠正的负参考，并以此微调 LVLM 主干。实验结果显示，SURf 显著提升了 LVLMs 在多模态参考利用上的效率，并增强了其对干扰信息的抵抗力。源代码已公开，详见 https://github.com/GasolSun36/SURf。
