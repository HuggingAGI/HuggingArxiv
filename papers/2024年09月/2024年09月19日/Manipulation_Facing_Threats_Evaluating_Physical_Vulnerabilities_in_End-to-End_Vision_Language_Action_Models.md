...
> 随着多模态大型语言模型 (MLLM) 的进步，视觉语言动作模型 (VLAM) 应运而生，旨在提升机器人操作任务在开放词汇场景中的表现。然而，操作任务涉及与物理世界的直接互动，确保任务执行中的鲁棒性和安全性至关重要。本文通过整合当前 MLLM 的安全研究与操作任务在物理世界中的应用场景，全面评估了 VLAM 在面对潜在物理威胁时的表现。我们提出了物理脆弱性评估管道 (PVEP)，该管道尽可能多地纳入视觉模态物理威胁，以评估 VLAM 的物理鲁棒性。PVEP 涵盖了分布外、基于排版的视觉提示和对抗性补丁攻击等威胁。通过对比攻击前后 VLAM 的性能波动，我们深入分析了 VLAM 对不同物理安全威胁的响应机制。
