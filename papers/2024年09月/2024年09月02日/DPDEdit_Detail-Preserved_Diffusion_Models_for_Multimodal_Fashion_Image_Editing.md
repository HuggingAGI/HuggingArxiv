...
> 时尚图像编辑是设计师展现创意的关键手段，通过互动式可视化设计概念来传达想法。尽管现有技术在多模态提示和扩散模型方面有所进步，但仍难以精准定位编辑区域并保留服装纹理细节。为此，我们推出了基于潜在扩散模型的新架构——细节保留扩散模型（DPDEdit），它通过融合文本提示、区域掩码、人体姿态及服装纹理图像，优化图像生成过程。我们利用Grounded-SAM根据文本描述精准预测编辑区域，并结合其他条件进行局部编辑。此外，我们设计了纹理注入与细化机制，通过解耦交叉注意力层和辅助U-Net，确保纹理细节的精确转移。我们还扩展了VITON-HD数据集，生成包含纹理与文本的配对样本。实验证明，DPDEdit在图像质量和多模态输入一致性方面超越了现有技术。
