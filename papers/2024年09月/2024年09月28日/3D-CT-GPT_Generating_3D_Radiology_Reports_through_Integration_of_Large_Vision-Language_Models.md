...
> 在现代放射诊断中，医学图像分析至关重要，尤其是在医学影像数据爆炸性增长的背景下。自动化报告生成系统的需求日益迫切。尽管现有研究多聚焦于2D医学图像的机器学习和多模态语言模型，但3D医学图像的报告生成因数据稀缺和计算复杂性而较少探索。本文推出的3D-CT-GPT，是一款基于视觉问答的医学视觉语言模型，专为从3D CT扫描（特别是胸部CT）生成放射学报告而设计。实验证明，3D-CT-GPT在报告准确性和质量上显著超越现有方法。尽管目前方法有限，如部分开源的CT2Rep和开源的M3D，我们通过合理的数据转换和评估确保了公平比较。实验显示，3D-CT-GPT不仅提升了诊断准确性，还增强了报告的连贯性，成为临床放射学报告生成的有力工具。未来，我们将扩展数据集并优化模型，以进一步提升其性能和应用范围。
