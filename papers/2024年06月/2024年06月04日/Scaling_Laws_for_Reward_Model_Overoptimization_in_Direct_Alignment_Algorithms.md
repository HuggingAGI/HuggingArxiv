...
这篇论文主要探讨了大型语言模型（LLMs）在人类反馈强化学习（RLHF）中的应用问题，特别是直接对齐算法（DDAs）在避免奖励过度优化或破解方面的挑战。论文通过实验研究了DDAs在不同训练目标、制度和模型规模下的表现，并分析了其退化趋势。这些问题和分析属于LLM的理论研究范畴，因此应归类为LLM理论。` `人工智能` `机器学习`

> Scaling Laws for Reward Model Overoptimization in Direct Alignment Algorithms

# 摘要

> 人类反馈强化学习（RLHF）对大型语言模型（LLMs）的成功至关重要，但这一过程复杂且易碎。传统RLHF框架中，首先训练奖励模型以体现人类偏好，再由在线强化学习算法利用此模型优化LLM。然而，这种方法常导致奖励过度优化或破解，即表面性能提升，实际质量却停滞或下降。直接对齐算法（DDAs）如直接偏好优化作为替代方案出现，避免了奖励建模阶段。尽管DDAs不依赖单独的代理奖励模型，它们仍因过度优化而退化。我们发现，在更高的KL预算下，DAA算法与传统RLHF有相似的退化趋势，甚至在数据集的一个周期完成前就开始退化。通过大量实验，本研究明确了DDAs中的奖励过度优化或破解问题，并探讨了其在不同训练目标、制度和模型规模下的影响。
