# 任务导向查询基准（ToQB），一个专为评估任务导向查询性能而设计的基准。

发布时间：2024年06月05日

`Agent

这篇论文主要关注的是如何构建一个任务导向型查询基准（ToQB），以评估虚拟助手、聊天机器人等LLM服务的质量。论文提出了一种方法，利用现有对话数据集和LLM服务来高效构建这样的基准。这种方法涉及制定NLP任务，提炼对话中说话者的意图，详细阐述使用LLM服务执行任务的步骤，并设计框架以自动化基准生成的大部分过程。因此，这篇论文更偏向于Agent的范畴，因为它专注于如何通过任务导向型查询来评估和改进虚拟助手等智能代理的性能。` `虚拟助手` `聊天机器人`

> The Task-oriented Queries Benchmark (ToQB)

# 摘要

> 任务导向型查询，如播放视频、订餐或叫车，是评估虚拟助手、聊天机器人等LLM服务质量的关键。尽管如此，针对此类查询的标准基准尚未建立，因为现有NLP领域的基准多聚焦于任务导向对话。为此，我们创新性地提出了一种方法，利用现有对话数据集和LLM服务，高效构建任务导向型查询基准（ToQB）。该方法涉及制定NLP任务，提炼对话中说话者的意图，详细阐述使用LLM服务执行任务的步骤，并设计框架以自动化基准生成的大部分过程。通过涵盖两个单任务领域和一个多任务领域的案例研究，我们展示了如何针对不同领域定制LLM提示，并分析了生成的任务导向型查询。ToQB数据集现已公开，我们亦探讨了社区可扩展的新领域及其应用前景。
