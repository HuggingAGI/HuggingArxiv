# GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配

发布时间：2024年06月03日

`LLM应用

这篇论文摘要讨论了在大型语言模型（LLM）的背景下，如何利用零-shot和few-shot学习来实现模式匹配，特别是在数据库系统中保护客户数据的身份和隐私。这种方法与传统的需要分析整个数据表的方法不同，更加注重数据安全和隐私保护。因此，这篇论文属于LLM应用类别，因为它探讨了LLM在特定应用场景（即数据库模式匹配）中的实际应用和影响。` `数据库` `数据分析`

> GRAM: Generative Retrieval Augmented Matching of Data Schemas in the Context of Data Security

# 摘要

> 模式匹配是现代数据库系统中数据摄取的关键环节，旨在发现两个不同数据表属性间的相似性。这一挑战在数据分析初期尤为突出，比如在整合第三方数据以洞察商业时。鉴于其在数据库领域的重要性，模式匹配自2000年代起便受到关注。本研究在大型语言模型的背景下重新审视这一问题，并遵循日益严格的数据安全政策，专注于零-shot和few-shot场景：模型仅需分析极少量的客户数据即可完成匹配，这与传统审查整个数据表的方法不同。我们强调，零-shot或few-shot的假设对于保护客户数据的身份和隐私至关重要，即便可能牺牲一定的准确性。在严格要求下准确匹配属性的能力，使我们的研究在该领域独树一帜。
...
![GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配](../../../paper_images/2406.01876/x1.png)

![GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配](../../../paper_images/2406.01876/x2.png)

![GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配](../../../paper_images/2406.01876/x3.png)

![GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配](../../../paper_images/2406.01876/x4.png)

![GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配](../../../paper_images/2406.01876/x5.png)

![GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配](../../../paper_images/2406.01876/x6.png)

![GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配](../../../paper_images/2406.01876/x7.png)

![GRAM：数据安全背景下，生成式增强检索实现数据模式的精准匹配](../../../paper_images/2406.01876/x8.png)
