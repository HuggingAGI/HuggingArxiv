...
> 评估与评价长久以来都是人工智能（AI）和自然语言处理（NLP）中的关键难题。然而，传统方法，不管是基于匹配的还是基于嵌入的，常常难以判断细微属性，也无法给出令人满意的结果。大型语言模型（LLMs）的最新进展催生了“LLM 充当评判者”的范式，即借助 LLMs 在各类任务和应用中进行评分、排名或选择。本文对基于 LLM 的判断和评估展开了全面调研，为推进这一新兴领域提供了深入概览。我们先从输入和输出的视角给出了详尽定义。接着，引入了一个全面的分类体系，从评判什么、如何评判以及在哪里评判这三个维度来探究“LLM 作为评判者”。最后，我们整理了用于评估“LLM 作为评判者”的基准，突出了关键挑战和有前景的方向，旨在为这个充满希望的研究领域提供宝贵见解，激发未来的研究。有关“LLM 作为评判者”的论文列表和更多资源可在 url{https://github.com/llm-as-a-judge/Awesome-LLM-as-a-judge} 及 url{https://llm-as-a-judge.github.io} 找到。
