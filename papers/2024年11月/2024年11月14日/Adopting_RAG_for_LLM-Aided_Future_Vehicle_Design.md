...
> 在本文里，我们探究了将大型语言模型（LLMs）与检索增强生成（RAG）相融合，以强化汽车行业的自动化设计与软件开发。我们给出了两个案例研究：一个是标准化合规聊天机器人，另一个是设计副驾驶，它们都借助 RAG 来给出精准、具备上下文感知的回应。我们对四个 LLMs——GPT-4o、LLAMA3、Mistral 和 Mixtral 进行了评估，对比了它们的回答准确率和执行时间。我们的成果显示，尽管 GPT-4 性能卓越，但 LLAMA3 和 Mistral 在本地部署方面也展现出良好的能力，解决了汽车应用中的数据隐私问题。此研究凸显了 RAG 增强型 LLMs 在优化汽车工程的设计工作流程和合规性方面的潜力。
...
[Arxiv](https://arxiv.org/abs/2411.09590)